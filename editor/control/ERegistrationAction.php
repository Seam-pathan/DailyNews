...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if ($flag == 1) {
        ...
        if ($db->checkUserExists("editor_info", $username)) {

            $validateEmail = "already has an user with that username";

        } else {

            $flag = $db->addUser("editor_info", $firstname, $lastname, $gender, $DateOfBirth, $Religion, $PresentAddress, $PermanentAddress, $phone, $email, $username, $pwd);
